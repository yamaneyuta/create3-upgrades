...
const getProxyAdminAddress = async (proxyAddress: string) => {
    ...
    const proxyFactory = new ethers.ContractFactory(
        TransparentUpgradeableProxyMeta.abi,
        TransparentUpgradeableProxyMeta.bytecode,
        ethers.provider,
    );
    ...
    const events = (await proxyContract.queryFilter(filter, blockNumber, blockNumber)).filter(
