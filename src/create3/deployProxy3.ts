...
const deployLogicContract = async (logicFactory: ContractFactory) => {
    ...
    //   - その他: `.openzeppelin`
    const { impl, kind, txResponse } = await deployProxyImpl(hre, logicFactory, {});
 ...
 * @param create3 
 */
const deployTransparentUpgradeableProxy = async(create3: Create3Upgradeable, salt: string, logic: Contract, logicFactory: ContractFactory, args: unknown[], opt?: Options) =>{
    const deployer = await getDeployer();
